...
class Snake {
	...
	public:
		Snake(int _dir, int _type, Point _p) {
			setDir(UP);
			setType(_type);
      ...
      if(len == 1) {
		...
		bool isBiteWall() {
			int body_X0 = body[0].getX();
			int body_Y0 = body[0].getY();
			return (abs(body_X0 - 15) <= 5 && dir == LEFT) || (abs(body_X0 - (MAX_X - 300)) <= 5 && dir == RIGHT) || (abs(body_Y0 - 20) <= 5 && dir == UP) || (abs(body_Y0 - (MAX_Y - 20)) <= 5 && dir == DOWN); 
		}
...
class Game {
	public:
		...
		void run() {
			...
			Snake fs(UP, FIRE_SNAKE, fsfp), ws(UP, WATER_SNAKE, wsfp);
			Circle food(200, 250, rand_in_range(4, 10, true));
			std::string fs_score = "", ws_score = "";
			CountdownTimer timer(1, 2);
			int c = 0, dct = 0; 
			bool reset_s = false; 
			bool fs_eat = false, ws_eat = false;
			bool fs_losed = false, ws_losed = false;
			bool eatEachotherCase = false;
			while(!fs_losed && !ws_losed && timer.getMinutes() >= 0) {
				if(fs_eat) fs_eat = false;
				if(ws_eat) ws_eat = false;
				if(eatEachotherCase) eatEachotherCase = false;
				setcolor(11);
				renderText(NULL, itos(timer.getMinutes()) + " : " + itos(timer.getSecond()), 6, MAX_X - 250, 90);
				setcolor(WHITE);
				renderText(NULL, "THOI GIAN CON LAI", 3, MAX_X - 265, 50);
				renderText(NULL, "HELLO WORLD", 3, MAX_X - 265, 150);
				if(c == 9) {
	  			c = 0;
	  			reset_s = false;
				...
				if(abs(fs.getBody()[0].getX() - food.getX()) < dct && abs(fs.getBody()[0].getY() - food.getY()) < dct) {
					fs.catchFood(food);
					fs_eat = true;
				}
				if(abs(ws.getBody()[0].getX() - food.getX()) < dct && abs(ws.getBody()[0].getY() - food.getY()) < dct) {
					ws.catchFood(food);
					ws_eat = true;
				}
				if(fs_eat || ws_eat) {
	      	...
	      	food.setR(rand_in_range(4, 10, true));
				...
				if(!eatEachotherCase || (!fs_losed && !ws_losed)) {
					if((fs.isBiteAnother(ws) || fs.isBiteWall() || fs.isBiteItself())) fs_losed = true;
					if((ws.isBiteAnother(fs) || ws.isBiteWall() || ws.isBiteItself())) ws_losed = true;
				}
				delay(100);
				c++;
				cleardevice();  
			}
			if(!fs_losed && !ws_losed) {
				int fs_len = fs.getLen(), ws_len = ws.getLen();
				std::cout << (fs_len > ws_len ? "FIRE SNAKE WIN" : (ws_len > fs_len ? "WATER SNAKE WIN" : "DRAW!")) << std::endl;
			}
			else {
				if(fs_losed && ws_losed) std::cout << "DRAW!" << std::endl;
				else if(fs_losed && !ws_losed) std::cout << "WATER SNAKE WIN" << std::endl;
				else std::cout << "FIRE SNAKE WIN" << std::endl;
			}
		}
		~Game() {}
};
